...
public class BufferedReader extends Reader
{
   ...
   */
  public BufferedReader(final Reader in, final int size)
  {
    super(in);
    if (size <= 0)
   ...
   * valid regarding buf.
   */
  public int read(final char[] buf, final int offset, int count) throws IOException
  {
   ...
   */
  private int lineEnd(final int limit)
  {
    int i = pos;
    for (; i < limit; i++)
      {
  ...
  public String readLine() throws IOException
  {
    ...
    if (i < limit)
      {
	final String str = String.valueOf(buffer, pos, i - pos);
	pos = i + 1;
