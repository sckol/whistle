...
public abstract class HtmlFilter {
	...
	public HtmlFilter(final Document doc, final String xpath)
	...
	protected boolean switchClass(final String newClassValue) {
		if (newClassValue.equals(currentElement.getAttribute("class")))
			return false;
		currentElement.setAttribute("class", newClassValue);
		return true;
	}
	
	protected boolean appendCssClass(final String className) {
		if (getCssClass().contains(" " + className)) return false;
		else {
			setCssClass(getCssClass() + " " + className);
			return true;
		}
	}
	
	protected boolean removeCssClass(final String className) {
		if (getCssClass().contains(" " + className)) {
			setCssClass(getCssClass().replace(" " + className, ""));
			return true;
		} else return false;
	}
	...
	protected boolean switchVisible(final boolean visible) {
		...
		if (visible == alreadyVisible)
			...
			currentElement.setAttribute("style",
					style.replaceFirst(";display:none;", ""));
		} else {
	...
	protected boolean switchText(final String text) {
		if (text.equals(currentElement.getText()))
			return false;
